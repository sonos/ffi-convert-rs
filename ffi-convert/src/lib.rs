...
//!   value drop (releasing its pointer fields via its [`Drop`] impl).
//!
//! ```text
...
//! // drops the value, releasing every inner pointer field via its `Drop`
//! // impl.
//! #[unsafe(no_mangle)]
...
//! macros. The `#[derive(CReprOf)]` macro also emits a [`Drop`] impl that
//! frees the type's string and pointer fields — only when it has some, so
//! types without owned memory can be `Copy`.
//!
//! # Deriving the traits
//!
...
//!   reads it to emit a null for `None` (and to skip the free on null when
//!   dropping) and [`AsRust`] to return `None` on a null pointer. A
