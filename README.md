...
`ffi-convert` provides two conversion traits — `CReprOf` (Rust → C, with the
cleanup hook that frees the heap data it owns) and `AsRust` (C → Rust) —
plus `RawPointerConverter` to handle pointer ownership, and derive macros
that take care of the boilerplate.
